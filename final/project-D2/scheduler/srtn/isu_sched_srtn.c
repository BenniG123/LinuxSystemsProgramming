...
isu_return_t srtn_add_task(void* _this, isu_task_t task)
{
	...
	//Assume list is sorted, as we exclusively build task list with this function
	
	//Insert new task into list based on remaining time
	isu_task_t task_compare = isu_llist_ittr_start(this->task_list, ISU_LLIST_HEAD);

	if(task_compare != NULL) {

		while((task_compare != NULL) && task_compare->remaining_time <= task->remaining_time) {
			task_compare = isu_llist_ittr_next(this->task_list);
		}
		if(task_compare == NULL) {
		...
		if(rv = isu_llist_push(this->task_list, task, ISU_LLIST_HEAD)) {
			isu_print(PRINT_ERROR, "Failed to add new task to the scheduler");
			return rv;
		}
	}
...
isu_task_t srtn_get_next_task(void* _this, int remove)
{
	...
	{
		isu_task_t task_compare = isu_llist_pop(this->task_list, ISU_LLIST_HEAD);

		//If the top of the list is shorter than the running task, run the top of the list
		//and add the previous running task back to the list
		if(task_compare->remaining_time < this->running_task->remaining_time) {
			isu_task_t insert_task = this->running_task;
			this->running_task = task_compare;
			srtn_add_task(this, insert_task);
			isu_print(PRINT_DEBUG, "Run different task");
		//Push back the top of the llist
		} else {
			isu_llist_push(this->task_list, task_compare,ISU_LLIST_HEAD);
	...
	else
	{
		...
		{
			isu_print(PRINT_DEBUG, "Run task which has the lowest time left");
		}
		else
		{
