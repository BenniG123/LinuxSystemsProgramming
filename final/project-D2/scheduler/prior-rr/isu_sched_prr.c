...
isu_task_t prr_get_next_task(void* _this, int remove)
{
	...
	// Running and not at quantum yet
	if (!remove && this->running_task && this->running_task->run_time < QUANTUM) {
		isu_print(PRINT_DEBUG, "Run same task again");
	}
	// Running and past quantum
	else if (!remove && this->running_task && this->running_task->run_time >= QUANTUM) {
		isu_print(PRINT_DEBUG, "Time has expired. Switching tasks.");
		prr_add_task(this, this->running_task);
		int highestPrio;
		for (i = 0; i < NUM_PRIORITIES; i++) { // Determine highest priority item
			if (isu_llist_count(this->task_list[i]) != 0) {
				highestPrio = i;
				break;
			}
		}
		this->running_task = isu_llist_pop(this->task_list[highestPrio], ISU_LLIST_HEAD);
		if (this->running_task) {
			isu_print(PRINT_DEBUG, "Run next task in round robin");
		}
		else {
			isu_print(PRINT_DEBUG, "No task to run, select IDLE task");
		}
	}
	// Finished, get new task
	else {
		isu_print(PRINT_DEBUG, "Time has expired. Switching tasks.");
		int highestPrio;
		for (i = 0; i < NUM_PRIORITIES; i++) { // Determine highest priority item
